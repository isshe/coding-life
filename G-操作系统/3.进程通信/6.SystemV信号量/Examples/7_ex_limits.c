// macOS 10.14.5测不出来: semget error: Invalid argument
...
#define	MAX_NPROC	isshe_sysconf(_SC_CHILD_MAX)
...
main(int argc, char **argv)
{
	...
	for (i = 0; i <= MAX_NIDS; i++) {
		sid[i] = semget(IPC_PRIVATE, 1, ISSHE_SVSEM_MODE | IPC_CREAT);
		if (sid[i] == -1) {
			semmni = i;
	...
	for (j = 0; j < i; j++)
		isshe_semctl(sid[j], 0, IPC_RMID);

		/* 4determine max # semaphores per semaphore set */
	for (i = 1; i <= MAX_MEMBERS; i++) {
		semid = semget(IPC_PRIVATE, i, ISSHE_SVSEM_MODE | IPC_CREAT);
		if (semid == -1) {
			semmsl = i-1;
	...
	printf("semmni = %d\n", semmni);
	for (i = 0; i < semmni; i++) {
		sid[i] = semget(IPC_PRIVATE, semmsl, ISSHE_SVSEM_MODE | IPC_CREAT);
		if (sid[i] == -1) {
/* $$.bp$$ */
			/*
			...
			for (j = semmsl-1; j > 0; j--) {
				sid[i] = semget(IPC_PRIVATE, j, ISSHE_SVSEM_MODE | IPC_CREAT);
				if (sid[i] != -1) {
					semmns += j;
					printf("max of %d semaphores\n", semmns);
					isshe_semctl(sid[i], 0, IPC_RMID);
					goto done;
				}
			}
			isshe_error_exit("j reached 0, semmns = %d", semmns);
		}
		semmns += semmsl;
	}
	printf("max of %d semaphores\n", semmns);
	...
	printf("semmsl = %d\n", semmsl);
	semid = isshe_semget(IPC_PRIVATE, semmsl, ISSHE_SVSEM_MODE | IPC_CREAT);
	for (i = 1; i <= MAX_NOPS; i++) {
		...
		if (semop(semid, ops, i)  == -1) {
			if (errno != E2BIG)
				isshe_sys_error_exit("expected E2BIG from semop");
			semopn = i-1;
	...
	semid = isshe_semget(IPC_PRIVATE, 1, ISSHE_SVSEM_MODE | IPC_CREAT);
	arg.val = semvmx;
	isshe_semctl(semid, 0, SETVAL, arg);		/* set value to max */
...
/* $$.bp$$ */
	...
	semid = isshe_semget(IPC_PRIVATE, 1, ISSHE_SVSEM_MODE | IPC_CREAT);
	arg.val = 0;
	isshe_semctl(semid, 0, SETVAL, arg);		/* set semaphore value to 0 */
	isshe_pipe(pipefd);
	child = isshe_malloc(MAX_NPROC * sizeof(pid_t));
	...
	for (i = 0; i < semmsl; i++) {
		arg.val = 0;
		isshe_semctl(semid, i, SETVAL, arg);		/* set semaphore value to 0 */
