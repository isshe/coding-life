...
int is_dhcp_packet(int sockfd)
{
    struct dhcp_packet packet;
    struct sockaddr_ll dest;
    socklen_t len;

    if (recvfrom(sockfd, &packet, sizeof(struct dhcp_packet), 0, (struct sockaddr *) &dest, &len) < 0) {
        printf("recvfrom error: %s\n", strerror(errno));
        exit(1);
    }

    if (ntohs(packet.udp.source) == SERVER_PORT && ntohs(packet.udp.dest) == CLIENT_PORT) {
        return 1;
    }
