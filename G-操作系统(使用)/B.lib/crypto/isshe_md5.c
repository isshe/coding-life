...
isshe_md5_body(isshe_md5_s *ctx, const uint8_t *data, uint64_t size);

void
isshe_md5_init(isshe_md5_s *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
isshe_md5_update(isshe_md5_s *ctx, const void *data, uint64_t size)
{
    uint64_t  used, free;

    used = (uint64_t) (ctx->bytes & 0x3f);
    ctx->bytes += size;
    ...
    if (used) {
        ...
        size -= free;
        (void) isshe_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = isshe_md5_body(ctx, data, size & ~(uint64_t) 0x3f);
        size &= 0x3f;
    }
...
void
isshe_md5_final(isshe_md5_s *ctx, uint8_t result[16])
{
    uint64_t  used, free;
    ...
    if (free < 8) {
        isshe_memzero(&ctx->buffer[used], free);
        (void) isshe_md5_body(ctx, ctx->buffer, 64);
        used = 0;
        free = 64;
    }

    isshe_memzero(&ctx->buffer[used], free - 8);
...
int isshe_md5(uint8_t *data, uint32_t len, uint8_t result[16])
{
    ...
    isshe_md5_init(&md5);
    isshe_md5_update(&md5, data, len);
    isshe_md5_final(&md5, result);
    return 0;
}
